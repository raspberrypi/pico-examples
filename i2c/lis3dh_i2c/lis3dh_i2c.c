...
void lis3dh_calc_value(uint16_t raw_value, float *final_value, bool isAccel){
    ...
    float scaling;
    float senstivity = 0.004; // g per unit
    ...
    // Check if value is < 0 and convert to decimal accordingly 
    if ((raw_value & 0x8000) == 0x8000) { 
        raw_value &= 0x7FFF; 
        *final_value = (-32768 + (float)raw_value) / scaling;
    } else {
        *final_value = (float)raw_value / scaling;
    }    
}
