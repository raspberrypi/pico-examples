...
static void doorbell_led_worker_fn(async_context_t *context, async_at_time_worker_t *worker) {
    int val = (int)worker->user_data;
    gpio_put(EXT_LED_GPIO_NUM, (val % 2) == 0);
    if (val < 11) {
        worker->user_data = (void*)(val + 1);
        async_context_add_at_time_worker_in_ms(context, worker, 100);
    }
}
static async_at_time_worker_t doorbell_led_worker = { .do_work = doorbell_led_worker_fn };
        ...
        case TC_W4_READY:
            switch(hci_event_packet_get_type(packet)) {
                case GATT_EVENT_NOTIFICATION: {
                    ...
                    if (value_length == 2) {
                        uint state = little_endian_read_16(value, 0);
                        INFO_LOG("Doorbell state %d \n", state);
                        if (state) {
                            // flash LED
                            doorbell_led_worker.user_data = 0;
                            async_context_add_at_time_worker_in_ms(cyw43_arch_async_context(), &doorbell_led_worker, 0);
                        }
                    } else {
...
static void hci_event_handler(uint8_t packet_type, uint16_t channel, uint8_t *packet, uint16_t size) {
    ...
    switch(event_type){
        ...
        case GAP_EVENT_ADVERTISING_REPORT:
            ...
            if (!advertisement_report_contains_service(ORG_BLUETOOTH_SERVICE_BINARY_SENSOR, packet)) return;
...
int main() {
    ...
    gpio_init(EXT_LED_GPIO_NUM); 
    gpio_set_dir(EXT_LED_GPIO_NUM, GPIO_OUT);
